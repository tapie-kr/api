...
import { ApplyFormDto, CreateApplyFormDto, UpdateApplyFormDto } from '../dto/form.dto';
import { ApplyForm, FormResponse, MemberUnit, Prisma } from '@tapie-kr/api-database/client';
import { PrismaService } from 'src/common/prisma/prisma.service';
import { FindFormsQueryDto } from '../dto/find-form-query-deprecated.dto';
...
export class ApplyFormRepository {
  ...
  async create(data: CreateApplyFormDto): Promise<ApplyForm> {
    try {
      return await this.prisma.applyForm.create({
        data,
      });
    } catch (error) {
  ...
  async remove(id: number): Promise<ApplyForm> {
    try {
      return await this.prisma.applyForm.delete({
        where: { id },
      });
    } catch (error) {
  ...
  async findAllResponses(id: number): Promise<{ responses: FormResponse[] }> {
    return this.prisma.applyForm.findUnique({
      where: { id },
      include: {
        responses: true,
